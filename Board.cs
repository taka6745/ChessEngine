...
namespace TSCP_Sharp
{
    public partial class TSCP
    {
        ...
        void set_hash()
        {
            ...
            for (i = 0; i < 64; ++i)
                if (color[i] != EMPTY)
                    hash ^= hash_piece[(6*64*color[i]) + (piece[i]*64) + i];
            if (side == DARK)
                hash ^= hash_side;
            if (ep != -1)
                hash ^= hash_ep[ep];
                ...
                if (color[i] == s)
                {
                    if (piece[i] == PAWN)
                    {
                        ...
                        for (j = 0; j < offsets[piece[i]]; ++j)
                            for (n = i; ; )
                            {
                                n = mailbox[mailbox64[n] + offset[piece[i]* 8 + j]];
                                if (n == -1)
                                    break;
                                if (n == sq)
                                    return true;
        ...
        void gen_push(int from, int to, int bits)
        {
            ...
            if (color[to] != EMPTY)
                ...
                g.score = history[from*64 + to];
        }

        /* gen_promote() is just like gen_push(), only it puts 4 moves
        n the move stack, one for each possible promotion piece */
