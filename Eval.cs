...
namespace TSCP_Sharp
{
    public partial class TSCP
    {
        ...
        const int IL = LIGHT * 10;
        const int ID = DARK * 10; 
        /* the values of the pieces */
        int[] piece_value= new int[6] {
        ...
        int[] king_endgame_pcsq=new int[64]{
        ...
        int[] pawn_rank=new int[2*10];

        int[] piece_mat=new int[2];  /* the value of a side's pieces */
        int[] pawn_mat = new int[2];  /* the value of a side's pawns */
        ...
        int eval()
        {
          ...
          for (i = 0; i < 64; ++i)
          {
              ...
              if (piece[i] == PAWN)
              {
                  ...
                  if (color[i] == LIGHT)
                  {
                      if (pawn_rank[IL + f] < (i >> 3))
                          pawn_rank[IL + f] = (i >> 3);
                  }
                  else
                  {
                      if (pawn_rank[ID + f] > (i >> 3))
                          pawn_rank[ID + f] = (i >> 3);
                  }
              }
              else
          ...
          for (i = 0; i < 64; ++i)
          {
              ...
              {  
                  ...
                  {
                      case PAWN:
                        score[LIGHT] += eval_light_pawn(i, i>>3);
                          break;
                      case KNIGHT:
                       ...
                       case BISHOP:
                          ...
                          break;
                       case ROOK:
                          if (pawn_rank[IL + ((i & 7) + 1)] == 0)
                          {
                              if (pawn_rank[ID + ((i & 7) + 1)] == 7)
              ...
              else
              {
                  ...
                  {
                      case PAWN:
                          score[DARK] += eval_dark_pawn(i, i >> 3);
                          break;
                      case KNIGHT:
        ...
        int eval_light_pawn(int sq, int row)
        {
            ...
            if (pawn_rank[IL + f] > row)
                r -= DOUBLED_PAWN_PENALTY;

            /* if there aren't any friendly pawns on either side of
               this one, it's isolated */
            if ((pawn_rank[IL + (f - 1)] == 0) &&
                    (pawn_rank[IL + (f + 1)] == 0))
                r -= ISOLATED_PAWN_PENALTY;

            /* if it's not isolated, it might be backwards */
            else if ((pawn_rank[IL + (f - 1)] < row) &&
                    (pawn_rank[IL + (f + 1)] < row))
                r -= BACKWARDS_PAWN_PENALTY;

            /* add a bonus if the pawn is passed */
            if ((pawn_rank[ID + (f - 1)] >= row) &&
                    (pawn_rank[ID + f] >= row) &&
                    (pawn_rank[ID + (f + 1)] >= row))
                r += (7 - row) * PASSED_PAWN_BONUS;

            return r;
        }

        int eval_dark_pawn(int sq, int row)
        {
            int r;  /* the value to return */
            int f;  /* the pawn's file */
            ...
            /* if there's a pawn behind this one, it's doubled */
            if (pawn_rank[ID + f] < row)
                r -= DOUBLED_PAWN_PENALTY;

            /* if there aren't any friendly pawns on either side of
               this one, it's isolated */
            if ((pawn_rank[ID + (f - 1)] == 7) &&
                    (pawn_rank[ID + (f + 1)] == 7))
                r -= ISOLATED_PAWN_PENALTY;

            /* if it's not isolated, it might be backwards */
            else if ((pawn_rank[ID + (f - 1)] > row) &&
                    (pawn_rank[ID + (f + 1)] > row))
                r -= BACKWARDS_PAWN_PENALTY;

            /* add a bonus if the pawn is passed */
            if ((pawn_rank[IL + (f - 1)] <= row) &&
                    (pawn_rank[IL + f] <= row) &&
                    (pawn_rank[IL + (f + 1)] <= row))
        ...
        int eval_lkp(int f)
        {
            ...
            if (pawn_rank[IL + f] == 6) ;  /* pawn hasn't moved */
            else if (pawn_rank[IL + f] == 5)
                r -= 10;  /* pawn moved one square */
            else if (pawn_rank[IL + f] != 0)
            ...
            if (pawn_rank[ID + f] == 7)
                r -= 15;  /* no enemy pawn */
            else if (pawn_rank[ID + f] == 5)
                r -= 10;  /* enemy pawn on the 3rd rank */
            else if (pawn_rank[ID + f] == 4)
        ...
        int eval_dkp(int f)
        {
            ...
            if (pawn_rank[ID + f] == 1) ;
            else if (pawn_rank[ID + f] == 2)
                r -= 10;
            else if (pawn_rank[ID + f] != 7)
                r -= 20;
            else
                r -= 25;
            ...
            if (pawn_rank[IL + f] == 0)
                r -= 15;
            else if (pawn_rank[IL + f] == 2)
                r -= 10;
            else if (pawn_rank[IL + f] == 3)
                r -= 5;
